...
public class HomeAdminCommand extends ContainerCommand
{
    ...
    public CommandResult run(CommandContext context) throws Exception
    {
        if (context.isSender(User.class))
        {
            ...
            if (user == null)
            {
                sender.sendTranslated(MessageType.NEGATIVE, "&2%s is not an user on this server!", context.getString(0));
                return null;
            }
            ...
            if (context.getArgCount() == 2)
            {
                ...
                if (home == null)
                {
                    sender.sendTranslated(MessageType.NEGATIVE, "&2%s does not have a home named &6%s&c!", user.getName(), context.getString(1));
                    return null;
                }
            }
            else
            {
                ...
                if (home == null)
                {
                    sender.sendTranslated(MessageType.NEGATIVE, "&2%s does not have a home!", user.getName());
                    return null;
                }
            }
            ...
            {
                sender.sendTranslated(MessageType.POSITIVE, "You have been teleported to &2%s&a's home!", user.getName());
            }
            return null;
        }
        else
    ...
    public CommandResult clear(final ParameterizedContext context)
    {
        ...
        if (context.getArgCount() > 0)
        {
            ...
            {
                context.sendTranslated(MessageType.NEGATIVE, "&2%s Isn't an user on this server", context.getString(0));
                return null;
            }
            else
            {
                ...
                {
                    context.sendTranslated(MessageType.NEUTRAL, "Are you sure you want to delete all public homes ever created by &2%s?", context.getString(0));
                    context.sendTranslated(MessageType.NEUTRAL, "To delete all the public homes, do: &6\"/confirm\" &ebefore 30 seconds has passed");
                }
                else if (context.hasFlag("priv"))
                {
                    context.sendTranslated(MessageType.NEUTRAL, "Are you sure you want to delete all private homes ever created by &2%s?", context.getString(0));
                    context.sendTranslated(MessageType.NEUTRAL, "To delete all the private homes, do: &6\"/confirm\" &ebefore 30 seconds has passed");
                }
                else
                {
                    context.sendTranslated(MessageType.NEUTRAL, "Are you sure you want to delete all homes ever created by &2%s?", context.getString(0));
                    context.sendTranslated(MessageType.NEUTRAL, "To delete all the homes, do: &6\"/confirm\" &ebefore 30 seconds has passed");
                }
            }
        }
        else
        {
            if (context.hasFlag("pub"))
            {
                ...
                context.sendTranslated(MessageType.NEUTRAL, "To delete all the public homes of every user, do: &6\"/confirm\" &ebefore 30 seconds has passed");
            }
            else if (context.hasFlag("priv"))
            {
                context.sendTranslated(MessageType.NEUTRAL, "Are you sure you want to delete all private homes ever created on this server?");
                context.sendTranslated(MessageType.NEUTRAL, "To delete all the private homes of every user, do: &6\"/confirm\" &ebefore 30 seconds has passed");
            }
            else
            {
                context.sendTranslated(MessageType.NEUTRAL, "Are you sure you want to delete all homes ever created on this server!?");
                context.sendTranslated(MessageType.NEUTRAL, "To delete all the homes of every user, do: &6\"/confirm\" &ebefore 30 seconds has passed");
            }
        }
    ...
    public void list(ParameterizedContext context)
    {
        ...
        else
        {
            ...
            if (user == null)
            {
                context.sendTranslated(MessageType.NEGATIVE, "Can't find any user named &2%s", context.getString(0));
                return;
            }
        ...
        for (Home home : homes)
        {
            if (home.isPublic())
            {
                context.sendTranslated(MessageType.NEUTRAL, "  &2public&e:&6%s", home.getName());
            }
            else
            {
                context.sendTranslated(MessageType.NEUTRAL, "  &2%s&e:&6%s", home.getOwnerName(), home.getName());
            }
        }
    }
    ...
    public void makePrivate(CommandContext context)
    {
        ...
        if (home == null)
        {
            context.sendTranslated(MessageType.NEGATIVE, "Couldn't find &6%s", context.getString(0));
            return;
        }
        if (!home.isPublic())
        {
            context.sendTranslated(MessageType.NEGATIVE, "&6%s is already private!", context.getString(0));
            return;
        }
        home.setVisibility(TeleportPointModel.VISIBILITY_PRIVATE);
        context.sendTranslated(MessageType.POSITIVE, "&6%s is now private", context.getString(0));
    }
    ...
    public void makePublic(CommandContext context)
    {
        ...
        if (home.isPublic())
        {
            context.sendTranslated(MessageType.NEGATIVE, "&6%s is already public!", context.getString(0));
            return;
        }
        home.setVisibility(TeleportPointModel.VISIBILITY_PUBLIC);
        context.sendTranslated(MessageType.POSITIVE, "&6%s is now public", context.getString(0));
    }
}
