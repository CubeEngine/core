...
public class AuctionManager
{
    ...
    public boolean undobid(Auction auction, Bidder bidder)
    {
        ...
        {
            user.sendTMessage("&6This is your auction!");
            return false;
        }
        if (!auction.isTopBidder(bidder))
        {
            user.sendTMessage("&6You are not the highest Bidder!");
            return false;
        }
        if (config.undotime > 0)
        {
            ...
            {
                user.sendTMessage("&6You can not undo your bid that late!");
                return false;
            }
        }
        auction.popBid();
        //TODO update DB
        user.sendTMessage("&aBid on Auction #%d redeemed!", auction.getKey());
        return true;
    ...
    public void endAuction(Auction auction)
    {
        if (auction.isTopBidder(auction.getOwner()))
        {
            ...
            user.sendTMessage("&6You have been charged &c%d%% &6of your startbid", config.comission);
    ...
    private void finishAuction(Auction auction, HashSet<Bidder> punished)
    {
        ...
        if (1 == 0)
        {
            ...
            User user = bidder.getUser();
            user.sendTMessage("&6Not enough money to pay what you bid for!");
            user.sendTMessage("&6You have been charged %d%% of your Bid.", config.punish);
            user.sendTMessage("&6Next time do not bid if you know you can not spare the money!");
            //TODO punish player
            auction.popBid();
            punished.add(bidder);
            //get next Winner
        ...
        seller.sendTMessage("&aCongratulations! &6You just sold: %s for %s excluding %s");//TODO params
        ...
        if (bidder.getUser().isOnline())
        {
            switch (notifyState)
            {
                case Bidder.NOTIFY_CANCEL:
                    bidder.getUser().sendTMessage("&6Your auction was not successful!");
                    return;
                case Bidder.NOTIFY_WIN:
                    bidder.getUser().sendTMessage("Congratulations! You just bought: %s for %s"); //TODO params
                    return;
            }
            return;
        }
