...
public class SpawnCommands
{
    ...
    public void setSpawn(CommandContext context)
    {
        ...
        else
        {
            if (sender == null)
            {
                ...
                context.sendTranslated(MessageType.NEUTRAL, "Use &6\"global\"&e instead of the role-name to set the default spawn.");
                return;
            }
        ...
        if (context.hasArg(0))
        {
            ...
            if (role == null)
            {
                ...
                {
                    context.sendTranslated(MessageType.NEGATIVE, "Could not find the role &6%s&c in &6%s&c!", context.getString(0), world.getName());
                    return;
                }
            }
            else
        ...
        context.sendTranslated(MessageType.POSITIVE, "The spawn in &6%s&a is now set to &eX:&6%d &eY:&6%d &eZ:&6%d", world.getName(), x.intValue(), y.intValue(), z.intValue());
    }
    ...
    public void spawn(ParameterizedContext context)
    {
        ...
        if (context.hasParam("world"))
        {
            ...
            if (world == null)
            {
                context.sendTranslated(MessageType.NEGATIVE, "World &6%s&c not found!", context.getString("world"));
                return;
            }
        }
        if (world == null)
        ...
        if (context.hasFlag("a"))
        {
            ...
            for (User player : context.getCore().getUserManager().getOnlineUsers())
            {
                ...
                if (rolespawn != null)
                {
                    ...
                    {
                        context.sendTranslated(MessageType.NEGATIVE, "Invalid spawn-location for the role of &2%s&c! &ePlease check your role-configurations!\n&7%s", player.getName(), rolespawn);
                        return;
                    }
        ...
        {
            context.sendTranslated(MessageType.NEGATIVE, "&6ProTip: &cTeleport does not work IRL!");
            return;
        }
        if (context.hasArg(0))
        {
            ...
            {
                context.sendTranslated(MessageType.NEGATIVE, "You are not allowed to spawn %s!", user.getName());
                return;
            }
        }
        final Location spawnLocation;
        if (context.hasParam("role"))
        {
            ...
            if (role == null)
            {
                context.sendTranslated(MessageType.NEGATIVE, "Could not find the role &6%s&c in &6%s&c!", roleName, world.getName());
                return;
            }
            String rolespawn = role.getRawMetadata().get("rolespawn");
            if (rolespawn == null)
            {
                context.sendTranslated(MessageType.NEGATIVE, "The role &6%s&c in &6%s&c has no spawn-point!", role.getName(), world.getName());
                return;
            }
            spawnLocation = this.getSpawnLocation(rolespawn);
            if (spawnLocation == null)
            {
                context.sendTranslated(MessageType.NEGATIVE, "Invalid spawn-location for the role &6%s&c! &ePlease check your role-configuration!\n&7%s", role.getName(), rolespawn);
                return;
            }
            context.sendTranslated(MessageType.POSITIVE, "You are now standing at the spawn of &6%s&a!", role.getName());
        }
        else
        {
            ...
            else
            {
                ...
                if (spawnLocation == null)
                {
                    context.sendTranslated(MessageType.NEGATIVE, "Invalid spawn-location for your role! &ePlease check your role-configurations!\n&7%s", rolespawn);
                    return;
                }
            }
        }
