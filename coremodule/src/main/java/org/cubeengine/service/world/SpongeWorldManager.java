...
public class SpongeWorldManager extends AbstractWorldManager implements WorldManager
{
    ...
    public void onEnable()
    {
        ...
        Result<WorldEntity> worldEntities = dsl.selectFrom(TableWorld.TABLE_WORLD).fetch();
        ...
        for (WorldEntity entity : worldEntities) // on serverstart this is empty (this is for reload)
        {
            ...
            if (world.isPresent())
            {
                if (loadedWorlds.contains(world.get()))
                {
                    ...
                    worldIds.put(entity.getValue(TableWorld.TABLE_WORLD.KEY), world.get());
        ...
        if (!loadedWorlds.isEmpty()) // new worlds?
        {
            ...
            {
                WorldEntity entity = dsl.newRecord(TableWorld.TABLE_WORLD).newWorld(world);
                entity.insertAsync();
                worlds.put(world.getName(), entity);
                worldIds.put(entity.getValue(TableWorld.TABLE_WORLD.KEY), world);
