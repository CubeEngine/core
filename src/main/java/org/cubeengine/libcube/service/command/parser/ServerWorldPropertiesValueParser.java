...
public class ServerWorldPropertiesValueParser implements ValueParser<ServerWorldProperties>, ValueCompleter, DefaultParameterProvider<ServerWorldProperties>
{
    ...
    @Override
    public Optional<? extends ServerWorldProperties> parseValue(Parameter.Key<? super ServerWorldProperties> parameterKey, ArgumentReader.Mutable reader,
            CommandContext.Builder context) throws ArgumentParseException
    {
        ...
        if (!world.isPresent())
        {
            final Optional<ServerWorldProperties> prop = Sponge.server().worldManager().loadProperties(key).join();
            if (prop.isPresent())
            {
                return prop;
            }
        ...
        return world.map(ServerWorld::properties);
    }
    ...
    @Override
    public ServerWorldProperties apply(CommandCause commandCause)
    {
        if (commandCause.audience() instanceof ServerPlayer)
        {
            return ((ServerPlayer)commandCause.audience()).world().properties();
        }
        return null;
    }
}
