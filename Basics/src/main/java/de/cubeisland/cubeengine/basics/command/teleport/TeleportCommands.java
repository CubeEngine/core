...
public class TeleportCommands
{
    ...
    public void tp(ParameterizedContext context)
    {
        ...
        if (context.hasArg(1)) //tp player1 player2
        {
            ...
            if (!force) // if force no need to check
            {
                if (user != context.getSender())
                {
                    ...
                    {
                        context.sendMessage("basics", "&cYou are not allowed to teleport &2%s&c!", user.getName());
                        return;
                    }
                ...
                if (target != context.getSender())
                {
                    if (BasicsPerm.TELEPORT_PREVENT_TPTO.isAuthorized(target)) // teleport to the target
                    {
                        ...
                        context.sendMessage("basics", "&cYou are not allowed to teleport to &2%s&c!", target.getName());
                        return;
                    }
        ...
        else
        {
            ...
            {
                context.sendMessage("basics", "&cTeleport to &4hell &cinitiated...");
                return;
            }
        }
        ...
        if (user.equals(target))
        {
            ...
            context.sendMessage("basics", "&aYou just teleported &2%s &ato &2%s... &eNot very useful right?", user.getName(), user.getName());
            return;
        }
    ...
    public void tpall(ParameterizedContext context)
    {
        ...
        if (!noTp.isEmpty())
        {
            context.sendMessage("basics", "&eThe following players were not teleported: \n&2%s",
              StringUtils.implode("&f,&2",noTp));
        }
    }
