...
public class Portal
{
    ...
    public void showInfo(CommandSender user)
    {
        ...
        user.sendTranslated(, "&aLocation: &6%d:%d:%d&a to &6%d:%d:%d&a in &6%s", this.config.location.from.x, this.config.location.from.y, this.config.location.from.z, this.config.location.to.x, this.config.location.to.y, this.config.location.to.z, this.config.world.getName());
        if (this.config.destination == null)
        {
        ...
        else
        {
            ...
            case WORLD:
                ...
                break;
            case LOCATION:
                user.sendTranslated(, "&aThis portal teleports to &6%d:%d:%d&a in &6%s", config.destination.location.x, config.destination.location.y, config.destination.location.z, config.destination.world.getName());
                break;
            }
