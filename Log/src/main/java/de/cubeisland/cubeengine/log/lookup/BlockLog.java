...
public class BlockLog implements Comparable<BlockLog>
{
    ...
    public void sendToUser(User user, boolean showLoc)
    {
        ...
        if (action == LogManager.BLOCK_BREAK)
        {
            ...
            list.add(Match.material().getNameFor(this.oldBlock.mat.getId(),this.oldBlock.data.shortValue()));
        }
        else if (action == LogManager.BLOCK_PLACE)
        {
            ...
            {
                message += "&eplaced &6%s";
                list.add(Match.material().getNameFor(this.newBlock.mat.getId(),this.newBlock.data.shortValue()));
            }
            else
            {
