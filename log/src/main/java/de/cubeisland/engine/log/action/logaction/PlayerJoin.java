...
public class PlayerJoin extends SimpleLogActionType
{
    ...
    protected void showLogEntry(User user, LogEntry logEntry, String time, String loc)
    {
        ...
        {
            user.sendTranslated(, "%s&2%s&a joined the server &6x%d%s", time, logEntry.getCauserUser().getDisplayName(), logEntry.getAttached().size() +1, loc);
        }
        else
        {
            user.sendTranslated(, "%s&2%s&a joined the server%s", time, logEntry.getCauserUser().getDisplayName(), loc);
        }
        //TODO ip if known
    }
