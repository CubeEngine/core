...
public class DoorUse extends BlockActionType
{
    ...
    protected void showLogEntry(User user, LogEntry logEntry, String time, String loc)
    {
        ...
        {
            user.sendTranslated(MessageType.POSITIVE, "%s&2%s &aopened the &6%s%s", time, logEntry.getCauserUser().getDisplayName(), logEntry.getOldBlock(), loc);
        }
        else
        {
            user.sendTranslated(MessageType.POSITIVE, "%s&2%s &aclosed the &6%s%s", time, logEntry.getCauserUser().getDisplayName(), logEntry.getOldBlock(), loc);
        }
    }
