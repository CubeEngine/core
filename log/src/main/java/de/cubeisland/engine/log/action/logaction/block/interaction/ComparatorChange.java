...
public class ComparatorChange extends BlockActionType
{
    ...
    protected void showLogEntry(User user, LogEntry logEntry, String time, String loc)
    {
        ...
        {
            user.sendTranslated(, "%s&2%s &aactivated the comparator%s", time, logEntry.getCauserUser().getDisplayName(), loc);
        }
        else
        {
            user.sendTranslated(, "%s&2%s &adeactivated the comparator%s", time, logEntry.getCauserUser().getDisplayName(), loc);
        }
    }
