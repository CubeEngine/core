...
public class TntExplode extends ExplosionActionType
{
    ...
    {
        return action instanceof TntExplode
            && this.oldBlock.equals(((TntExplode)action).oldBlock)
            && this.entity.equals(((TntExplode)action).entity)
            && ((this.player == null && ((TntExplode)action).player == null)
            || (this.player != null && this.player.equals(((TntExplode)action).player)));
    }
    ...
    public String translateAction(User user)
    {
        ...
        if (this.player == null)
        {
            return user.getTranslationN(POSITIVE, count,
                                        "A TNT-Explosion got rid of {name#block}",
                                        "A TNT-Explosion got rid of {1:amount}x {name#block}",
                                        this.oldBlock.name(), count);
        }
        return user.getTranslationN(POSITIVE, count,
                                    "A TNT-Explosion induced by {user} got rid of {name#block}",
