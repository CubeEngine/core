 ...
 * @author Faithcaio
 */
public class UserControl_old
{

    private Map<OfflinePlayer, User_old> users = new HashMap<OfflinePlayer, User_old>();
    private Server server = CubeWar.getInstance().getServer();
    private UserStorage userDB;

    public UserControl_old()
    {
    }
    ...
    public void loadDB()
    {
        ...
        for (User_old user : userDB.getAll())
        {
    ...
    {
        User_old killer = getUser(killerPlayer);
        User_old killed = getUser(killedPlayer);
    ...
    {
        User_old killer = getOfflineUser(killerPlayer);
        killer.kill(monster);
        userDB.update(killer);
    }

    public String getUserKD(User_old user, User_old killed, int kill)
    {
    ...
    public User_old getOfflineUser(OfflinePlayer user)
    {
        ...
        {
            User_old newuser = new User_old(user);
    ...
    public User_old getUser(CommandSender sender)
    {
    ...
    public User_old getUser(String name)
    {
    ...
    public Collection<User_old> getUsers()
    {
    ...
    {
        User_old user1 = getUser(player1);
        User_old user2 = getUser(player2);
