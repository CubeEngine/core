 ...
 * @author Faithcaio
 */
public class Group_old implements Cloneable, Model
{
    ...
    private List<User_old> adminlist = new ArrayList<User_old>();
    private List<User_old> modlist = new ArrayList<User_old>();
    private List<User_old> userlist = new ArrayList<User_old>();
    //TODO DB in separater Tabelle
    private List<User_old> invited = new ArrayList<User_old>();
    //TODO DB in separater Tabelle
    private List<Group_old> enemy = new ArrayList<Group_old>();
    private List<Group_old> ally = new ArrayList<Group_old>();
    ...
    public Group_old(int id, String tag, String name, String desc, boolean isarena, int respawnprot,
    ...
    @Override
    public Group_old clone()
    {
        Group_old group = new Group_old();
        group.getBits().reset();
        ...
        if (this.getBits().isset(Group_old.ECONOMY_BANK))
        {
            group.setBit(Group_old.ECONOMY_BANK);
        }
        group.setIntegerValue("power_perm", this.getPower_perm());
        group.setIntegerValue("power_boost", this.getPower_boost());
        if (this.getBits().isset(Group_old.POWER_LOSS))
        {
            group.setBit(Group_old.POWER_LOSS);
        }
        if (this.getBits().isset(Group_old.POWER_GAIN))
        {
            group.setBit(Group_old.POWER_GAIN);
        }
        if (this.getBits().isset(Group_old.PVP_ON))
        {
            group.setBit(Group_old.PVP_ON);
        }
        if (this.getBits().isset(Group_old.PVP_DAMAGE))
        {
            group.setBit(Group_old.PVP_DAMAGE);
        }
        if (this.getBits().isset(Group_old.PVP_FRIENDLYFIRE))
        {
            group.setBit(Group_old.PVP_FRIENDLYFIRE);
        }
        ...
        if (this.getBits().isset(Group_old.MONSTER_SPAWN))
        {
            group.setBit(Group_old.MONSTER_SPAWN);
        }
        if (this.getBits().isset(Group_old.MONSTER_DAMAGE))
        {
            group.setBit(Group_old.MONSTER_DAMAGE);
        }
        if (this.getBits().isset(Group_old.BUILD_DESTROY))
        {
            group.setBit(Group_old.BUILD_DESTROY);
        }
        if (this.getBits().isset(Group_old.BUILD_PLACE))
        {
            group.setBit(Group_old.BUILD_PLACE);
        }
        group.setListValue("protect", this.getProtect());
        if (this.getBits().isset(Group_old.USE_FIRE))
        {
            group.setBit(Group_old.USE_FIRE);
        }
        if (this.getBits().isset(Group_old.USE_LAVA))
        {
            group.setBit(Group_old.USE_LAVA);
        }
        if (this.getBits().isset(Group_old.USE_WATER))
        {
            group.setBit(Group_old.USE_WATER);
        }
    ...
    public void delAdmin(User_old user)
    {
        this.adminlist.remove(user);
        user.setTeam(groups.getWildLand());
    }

    public boolean isAdmin(User_old user)
    {
        return this.adminlist.contains(user);
    }

    public void addMod(User_old user)
    {
        if (user == null)
        {
            return;
    ...
    public void delMod(User_old user)
    {
        this.modlist.remove(user);
        this.adminlist.remove(user);
        user.setTeam(groups.getWildLand());
    ...
    public boolean isMod(User_old user)
    {
    ...
    public void delUser(User_old user)
    {
        this.userlist.remove(user);
        this.modlist.remove(user);
        this.adminlist.remove(user);
    ...
    public boolean isUser(User_old user)
    {
        if (this.adminlist.contains(user))
        {
            return true;
    ...
    public boolean invite(User_old user)
    {
    ...
    public boolean uninvite(User_old user)
    {
        if (this.invited.contains(user))
        {
            return false;
    ...
    public boolean isInvited(User_old user)
    {
    ...
    public boolean isBalanced(User_old user)
    {
        if (this.invited.contains(user))
        {
            return true; //Invited Player can always join!
        }
        return GroupControl_old.get().isBalanced(this);
    }
    ...
    public void adjustMaxInfluence()
    {
        ...
        for (User_old user : this.getUserList())
        {
    ...
    public void setneutral(Group_old g)
    {
        this.ally.remove(g);
        this.enemy.remove(g);
    }

    public void setally(Group_old g)
    {
        this.ally.add(g);
        this.enemy.remove(g);
    }

    public void setenemy(Group_old g)
    {
        this.ally.remove(g);
        this.enemy.add(g);
    }

    public boolean isAlly(Group_old g)
    {
        return ally.contains(g);
    }

    public boolean isTrueAlly(Group_old g)
    {
        if (this.equals(g))
        {
            return true;
    ...
    public boolean isEnemy(Group_old g)
    {
        return enemy.contains(g);
    }

    public boolean isneutral(Group_old g)
    {
    ...
    {
        this.sendToTeam(msg);
        for (Group_old theally : this.ally)
        {
    ...
    public void sendInfo(CommandSender sender)
    {
        ...
        sender.sendMessage(t("g_04", GroupControl_old.get().getRank(this),
        ...
        String allies = "";
        String enemies = "";
        for (Group_old group : groups.getGroups())
        {
        ...
        List<User_old> list = this.getUserList();
        sender.sendMessage(t("g_15", list.size(), this.getKPSum()));
        String onplayer = "";
        List<User_old> onlist = new ArrayList<User_old>();
        for (User_old player : list)
        {
    ...
    public List<User_old> getUserList()
    {
        List<User_old> list = new ArrayList<User_old>();
    ...
    {
        int kp = 0;
        for (User_old tmp : this.getUserList())
        {
            kp += tmp.getKp();
        }
        return kp;
