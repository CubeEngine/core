 ...
 * @author Faithcaio
 */
public class User_old implements Model
{
    ...
    public User_old(OfflinePlayer player)
    {
        this.user = cuManager.getCubeUser(player);
        rank = config.cubewar_ranks.get(0);
    }

    public User_old(int cubeuserid, int death, int kills, int kp, PlayerMode mode, int teamid, int teampos, double influence)
    {
    ...
    private int kill_kd(User_old user)
    {
    ...
    public void setTeam(Group_old team)
    {
        this.team = team;
        userDB.update(this);
    }

    public Group_old getTeam()
    {
        if (this.team == null)
        {
